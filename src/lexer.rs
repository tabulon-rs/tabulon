...
impl<'a> Lexer<'a> {
    ...
    pub(crate) fn next_token(&mut self) -> Result<Token, JitError> {
        ...
        match c {
            ...
            b'*' => { self.bump(); Ok(Token::Star) }
            b'/' => { self.bump(); Ok(Token::Slash) }
            b'=' => {
                self.bump();
                if self.peek() == Some(b'=') { self.bump(); Ok(Token::EqEq) } else { Err(JitError::Parse("expected '=' after '=' for '=='".into())) }
            }
            b'!' => {
                self.bump();
                if self.peek() == Some(b'=') { self.bump(); Ok(Token::NotEq) } else { Err(JitError::Parse("expected '=' after '!' for '!='".into())) }
            }
            b'<' => {
                self.bump();
                if self.peek() == Some(b'=') { self.bump(); Ok(Token::Le) } else { Ok(Token::Lt) }
            }
            b'>' => {
                self.bump();
                if self.peek() == Some(b'=') { self.bump(); Ok(Token::Ge) } else { Ok(Token::Gt) }
            }
            b'&' => {
                self.bump();
                if self.peek() == Some(b'&') { self.bump(); Ok(Token::AndAnd) } else { Err(JitError::Parse("expected '&' after '&' for '&&'".into())) }
            }
            b'|' => {
                self.bump();
                if self.peek() == Some(b'|') { self.bump(); Ok(Token::OrOr) } else { Err(JitError::Parse("expected '|' after '|' for '||'".into())) }
            }
    ...
    fn lex_number(&mut self) -> Result<Token, JitError> {
        ...
        let v: f64 = s.parse().map_err(|e| JitError::Parse(format!("invalid number '{}': {}", s, e)))?;
        Ok(Token::Num(v))
    }
    fn lex_ident(&mut self) -> Result<Token, JitError> {
        let start = self.i;
        while let Some(c) = self.peek() {
            if c.is_ascii_alphanumeric() || c == b'_' { self.i += 1; } else { break; }
        }
        let s = std::str::from_utf8(&self.src[start..self.i]).unwrap().to_string();
        if s.is_empty() { return Err(JitError::Parse("unexpected character".into())); }
        Ok(Token::Ident(s))
    }
}
