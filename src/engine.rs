...
pub enum VarAccessStrategy {
    ...
    /// Call an external resolver function with (unified_ctx, idx) to obtain each variable lazily,
    /// where unified_ctx contains both the resolver and the user context.
    ResolverAndCtxCall { symbol: &'static str },
}
...
impl<Ctx> Tabula<Ctx> {
    ...
    /// Convenience: enable the built-in trait-based resolver trampoline.
    ///
    /// After calling this, you can compile with VarAccessStrategy::ResolverCall using
    /// RESOLVER_TRAMPOLINE_SYMBOL, and evaluate with CompiledExpr::eval_with_resolver(...)
    /// without exposing a context type to your callers.
    pub fn enable_trait_resolver(&mut self) -> Result<(), JitError> {
        if self.module.is_some() {
            return Err(JitError::Internal(
                "cannot set var getter after JIT module is created; create a new Engine".into(),
            ));
        }
        self.var_getter = Some((
            crate::trait_resolver::RESOLVER_TRAMPOLINE_SYMBOL,
            crate::trait_resolver::resolve_var_tramp,
        ));
        Ok(())
    }
    ...
    fn ensure_module_and_register(&mut self) -> Result<(), JitError> {
        if self.module.is_none() {
            ...
            // Register the unified resolver trampoline
            jb.symbol(
                crate::trait_resolver::RESOLVER_AND_CTX_TRAMPOLINE_SYMBOL,
                crate::trait_resolver::resolve_var_tramp_unified::<Ctx> as *const u8,
            );
    ...
    fn build_and_finalize_with(
        ...
        let mut fb_ctx = FunctionBuilderContext::new();
        {
            ...
            let result_val = match strat {
                ...
                VarAccessStrategy::ResolverCall { symbol } | VarAccessStrategy::ResolverAndCtxCall { symbol } => {
    ...
    pub fn compile_prepared_with<K: Clone>(&mut self, prepared: &PreparedExpr<K>, strat: VarAccessStrategy) -> Result<CompiledExpr<K, Ctx>, JitError> {
        let (code, is_resolver) = match strat {
            ...
            VarAccessStrategy::ResolverCall { .. } | VarAccessStrategy::ResolverAndCtxCall { .. } => (
                self.build_and_finalize_with(
                    &prepared.var_index,
                    &prepared.ast,
                    prepared.needs_bool_consts,
        ...
        Ok(CompiledExpr::<K, Ctx> {
            ...
            gen_at_compile: gen_at,
            uses_ctx: ast_uses_ctx(&*prepared.ast, &self.funcs),
            resolver_mode: matches!(strat, VarAccessStrategy::ResolverCall { .. } | VarAccessStrategy::ResolverAndCtxCall { .. }),
            _phantom_ctx: std::marker::PhantomData,
        })
    }
...
impl<K, Ctx> CompiledExpr<K, Ctx> {
    ...
    /// Evaluates a resolver-compiled expression by passing a trait-based resolver directly,
    /// hiding the internal resolver context from the caller.
    pub fn eval_with_resolver(&self, resolver: &mut dyn crate::trait_resolver::Resolver) -> Result<f64, JitError> {
        if self.gen_token.load(Ordering::Relaxed) != self.gen_at_compile {
            return Err(JitError::Invalidated);
        }
        if !self.resolver_mode {
            return Err(JitError::Internal("eval_with_resolver called on non-resolver compiled expr".into()));
        }
        let rctx = crate::trait_resolver::ResolverCtx { resolver };
        let f = self.func_ptr;
        let ctx_ptr = (&rctx as *const crate::trait_resolver::ResolverCtx) as crate::rt_types::CtxPtr;
        let out = unsafe { f(std::ptr::null(), ctx_ptr) };
        Ok(out)
    }

    /// Evaluates a resolver-compiled expression by passing both a trait-based resolver and a user context.
    /// This method uses a `UnifiedContext` internally to pass both the resolver and the user context
    /// to the JIT-compiled function, allowing registered functions to access the user context
    /// and variable resolution to use the resolver.
    pub fn eval_with_resolver_and_ctx(&self, resolver: &mut dyn crate::trait_resolver::Resolver, ctx: &mut Ctx) -> Result<f64, JitError> {
        if self.gen_token.load(Ordering::Relaxed) != self.gen_at_compile {
            return Err(JitError::Invalidated);
        }
        if !self.resolver_mode {
            return Err(JitError::Internal("eval_with_resolver_and_ctx called on non-resolver compiled expr. Compile with ResolverAndCtxCall strategy.".into()));
        }
        let uctx = crate::rt_types::UnifiedContext { resolver, user_ctx: ctx };
        let f = self.func_ptr;
        let ctx_ptr = (&uctx as *const crate::rt_types::UnifiedContext<Ctx>) as crate::rt_types::CtxPtr;
        let out = unsafe { f(std::ptr::null(), ctx_ptr) };
        Ok(out)
    }
}
...
impl<K, Ctx> CompiledExprRef<K, Ctx> {
    ...
    /// Evaluates a resolver-compiled expression by passing a trait-based resolver directly
    /// (pointer/ref input mode). This hides the internal resolver context from the caller.
    pub fn eval_with_resolver(&self, resolver: &mut dyn crate::trait_resolver::Resolver) -> Result<f64, JitError> {
        if self.gen_token.load(Ordering::Relaxed) != self.gen_at_compile {
            return Err(JitError::Invalidated);
        }
