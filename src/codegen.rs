...
pub(crate) fn codegen_expr<'a>(
    ...
    match ast {
        ...
        Ast::Var(name) => {
            ...
            Ok(ensure_var(
                builder, vars_ptr, ptr_ty, mf, idx, cache, load_mode,
            ))
        }
        Ast::Neg(x) => {
            let v = codegen_expr(
                module, registry, builder, var_index, vars_ptr, ptr_ty, mf, cache, x, f64_ty,
                load_mode,
            )?;
        ...
        Ast::Add(a, b) => {
            let va = codegen_expr(
                module, registry, builder, var_index, vars_ptr, ptr_ty, mf, cache, a, f64_ty,
                load_mode,
            )?;
            let vb = codegen_expr(
                module, registry, builder, var_index, vars_ptr, ptr_ty, mf, cache, b, f64_ty,
                load_mode,
            )?;
        ...
        Ast::And(a, b) => {
            ...
            builder
                .ins()
                .jump(merge_block, &[BlockArg::Value(zero_else)]);
        ...
        Ast::If(c, t, e) => {
            ...
            let pre_cache = cache.clone();
            let vc = codegen_expr(
                module, registry, builder, var_index, vars_ptr, ptr_ty, mf, cache, c, f64_ty,
                load_mode,
            )?;
            let one = builder.ins().f64const(1.0);
