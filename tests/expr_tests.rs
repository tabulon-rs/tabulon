...
fn expr_relations_constants_and_vars() {
    ...
    let a = 1.0;
    // A < 2 => true
    let c = eng.compile("A < 2").unwrap();
    assert_eq!(c.eval(&[&a]).unwrap(), 1.0);

    // A <= 1 => true
    let c = eng.compile("A <= 1").unwrap();
    assert_eq!(c.eval(&[&a]).unwrap(), 1.0);
...
fn expr_precedence_mix() {
    ...
    let a = 1.0;
    let e = eng.compile("(A + 2) > 2 && 0 || 1").unwrap();
...
fn expr_if_builtin_constants() {
    let mut eng = Tabula::new();
    let e1 = eng.compile("if(1, 1, 2)").unwrap();
    assert_eq!(e1.eval(&[]).unwrap(), 1.0);

    let e2 = eng.compile("if(0, 1, 2)").unwrap();
    assert_eq!(e2.eval(&[]).unwrap(), 2.0);

    let e3 = eng.compile("if(2, 1, 2)").unwrap();
...
fn expr_builtin_max_constants() {
    let mut eng = Tabula::new();
    let e1 = eng.compile("max(1, 2) == 2").unwrap();
    assert_eq!(e1.eval(&[]).unwrap(), 1.0);

    let e2 = eng.compile("max(-1, 0) == 0").unwrap();
    assert_eq!(e2.eval(&[]).unwrap(), 1.0);

    let e3 = eng.compile("max(1.1111111111111, 1.1111111111112) == 1.1111111111112").unwrap();
    assert_eq!(e3.eval(&[]).unwrap(), 1.0);
}
...
fn expr_many_vars() {
    ...
    let e1 = eng.compile("A + B + C + D + E + F + G + H").unwrap();
